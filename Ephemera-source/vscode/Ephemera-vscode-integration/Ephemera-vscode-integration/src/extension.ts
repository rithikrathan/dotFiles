...
const getSchemeDir = () => join(process.env.XDG_STATE_HOME ?? join(homedir(), ".local", "state"), "Ephemera");
...
const update = async () => {
    ...
    await writeFile(join(__dirname, "..", "themes", "Ephemera.json"), JSON.stringify(theme(colours)));

    // Sync icon theme
    const workbench = workspace.getConfiguration("workbench");
    if (
        workbench.get("colorTheme") === "Ephemera" &&
