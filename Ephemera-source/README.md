...
This is the main repo of the Ephemera dots and contains the user configs for
...
```sh
git clone https://github.com/Ephemera-dots/Ephemera.git ~/.local/share/Ephemera
~/.local/share/Ephemera/install.fish
```
...
[shell](https://github.com/Ephemera-dots/shell) and [cli](https://github.com/Ephemera-dots/cli)
to install them.
...
```sh
spicetify config current_theme Ephemera color_scheme Ephemera custom_apps marketplace
spicetify apply
```
...
Finally, install the extension VSIX from `vscode/Ephemera-vscode-integration`.

```sh
# Use `codium` if using VSCodium
code --install-extension vscode/Ephemera-vscode-integration/Ephemera-vscode-integration-*.vsix
```
...
`~/.mozilla/native-messaging-hosts/Ephemerafox.json` and replacing the `{{ $lib }}` string in it
with the absolute path of `~/.local/lib/Ephemera` (this must be the absolute path, e.g.
`/home/user/.local/lib/Ephemera`). Then copy or symlink `zen/native_app/app.fish` to
`~/.local/lib/Ephemera/Ephemerafox`.

Finally, install the EphemeraFox extension from [here](https://addons.mozilla.org/en-US/firefox/addon/Ephemerafox).
