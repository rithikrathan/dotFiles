...
echo '╰─────────────────────────────────────────────────╯'
set_color normal
log 'Welcome to the Ephemera dotfiles installer!'
...
fish -c 'rm -f Ephemera-meta-*.pkg.tar.zst' 2> /dev/null
...
if set -q _flag_spotify
    ...
    if confirm-overwrite $config/spicetify
        ...
        spicetify config current_theme Ephemera color_scheme Ephemera custom_apps marketplace 2> /dev/null
        spicetify apply
    end
end
...
if set -q _flag_vscode
    ...
    if confirm-overwrite $folder/settings.json && confirm-overwrite $folder/keybindings.json && confirm-overwrite $config/$prog-flags.conf
        ...
        # Install extension
        $prog --install-extension vscode/Ephemera-vscode-integration/Ephemera-vscode-integration-*.vsix
    end
end
...
if set -q _flag_zen
    ...
    set -l lib $HOME/.local/lib/Ephemera

    if confirm-overwrite $hosts/Ephemerafox.json
        log 'Installing zen native app manifest...'
        mkdir -p $hosts
        cp zen/native_app/manifest.json $hosts/Ephemerafox.json
        sed -i "s|{{ \$lib }}|$lib|g" $hosts/Ephemerafox.json
    end

    if confirm-overwrite $lib/Ephemerafox
        log 'Installing zen native app...'
        mkdir -p $lib
        ln -s (realpath zen/native_app/app.fish) $lib/Ephemerafox
    end

    # Prompt user to install extension
    log 'Please install the EphemeraFox extension from https://addons.mozilla.org/en-US/firefox/addon/Ephemerafox if you have not already done so.'
end

# Generate scheme stuff if needed
if ! test -f $state/Ephemera/scheme.json
    Ephemera scheme set -n shadotheme
    sleep .5
    hyprctl reload
end

# Start the shell
Ephemera shell -d > /dev/null
